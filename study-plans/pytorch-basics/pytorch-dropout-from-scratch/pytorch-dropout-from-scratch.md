...
<span style="font-size: 14px;">A critical aspect of dropout is that it behaves differently during training and evaluation. In PyTorch, every</span> nn.Module <span style="font-size: 14px;">has a boolean attribute</span> self.training <span style="font-size: 14px;">that tracks the current mode:</span>

* <span style="font-size: 14px;">When</span> model.train() <span style="font-size: 14px;">is called,</span> self.training <span style="font-size: 14px;">is set to</span> True <span style="font-size: 14px;">for all submodules. Dropout is active.</span>
* <span style="font-size: 14px;">When</span> model.eval() <span style="font-size: 14px;">is called,</span> self.training <span style="font-size: 14px;">is set to</span> False <span style="font-size: 14px;">for all submodules. Dropout is disabled and the input passes through unchanged.</span>
...
* <span style="font-size: 14px;">**Bernoulli sampling**: Use</span> torch.bernoulli <span style="font-size: 14px;">to directly sample a tensor of 0s and 1s with the desired probability.</span>
* <span style="font-size: 14px;">**Uniform comparison**: Generate a tensor of uniform random values with</span> torch.rand <span style="font-size: 14px;">and compare against</span> $p$<span style="font-size: 14px;">. Elements where the random value exceeds</span> $p$ <span style="font-size: 14px;">are kept (set to 1), and the rest are set to 0. This is equivalent to Bernoulli sampling.</span>

<span style="font-size: 14px;">Both approaches produce the same distribution, but their individual masks can differ. The uniform comparison method is slightly more explicit:</span>
...
<span style="font-size: 14px;">When building dropout as an</span> nn.Module<span style="font-size: 14px;">, the key design decisions are:</span>

* <span style="font-size: 14px;">Store</span> $p$ <span style="font-size: 14px;">as an instance attribute in</span> __init__
* <span style="font-size: 14px;">Check</span> self.training <span style="font-size: 14px;">in</span> forward <span style="font-size: 14px;">to decide whether to apply dropout</span>
...
<span style="font-size: 14px;">The</span> self.training <span style="font-size: 14px;">flag is automatically managed by PyTorch when you call</span> .train() <span style="font-size: 14px;">or</span> .eval() <span style="font-size: 14px;">on the model. You do not need to set it manually.</span>
